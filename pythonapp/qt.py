...
def connect():
	#bd_addr = "7C:61:93:D1:BE:4C" #BlueSmirf address
...
class Login(QtGui.QDialog):
    ...
    def handleLogin(self):
        ...
        try:
            ...
            message = 'Logged in as ', str(self.textName.text())
...
class MainWidget(QtGui.QWidget):
    ...
    def gmailCheck(self, parent):
        ...
        try:
           ...
           if( unread > 0 ):
               update()
               self.window().statusBar().showMessage('Gmail Unread Count = ' + str(unread))
               self.status('Gmail Unread Count = ' + str(unread))
           else:
               clear()
               self.status('No unread e-mails.')
        except:
           self.window().statusBar().showMessage( 'Not logged in, check failed.')
           self.status('Please login to check e-mails.')
